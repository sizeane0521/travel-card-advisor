...
- `storeRules`: array of objects, each containing:
  ...
  - `subCategories` (optional): array of objects, each with:
    - `label`: sub-category label string as it appears on the page (e.g. "便利商店", "樂園", "百貨")
    - `stores`: array of store name strings belonging to this sub-category

The prompt SHALL instruct the model to populate `subCategories` only when the bank page visually groups stores under distinct sub-headings within the same bonus category. If no visual grouping exists, the field SHALL be omitted entirely (not an empty array).

The prompt SHALL include an example showing a `storeRules` entry with a non-empty `subCategories` array alongside the existing examples for `capPeriod`, `validFrom`, and `validTo`.

The `CardImportResult` type SHALL contain the fields: `cardName`, `baseRate`, `rewardCap`, `spendCap`, `validFrom`, `validTo`, and `storeRules` with the schema described above, including the optional `subCategories` field on each `storeRules` entry.
...
- **WHEN** the bank promotion page visually groups stores under sub-headings within one bonus category (e.g. "便利商店": 7-ELEVEN, FamilyMart; "樂園": 東京迪士尼, 大阪環球影城)
...
- **THEN** the form SHALL pre-fill the StoreBonus `subCategories` field with the extracted sub-groups
...
- **THEN** the form SHALL pre-fill `stores[]` only (existing behavior unchanged)
...
- **THEN** the `subCategories` field SHALL be omitted
...
- **THEN** the system SHALL parse the JSON response into card fields including `validFrom`, `validTo`, `storeRules` with `stores[]`, `capPeriod`, and optional `subCategories`
